...
class Pong:
    ...
    def getInit(self,render):
        ...
        if(render):
            ...
            puck1 = Rectangle(Point(self.posPuck1[0] - 7, self.posPuck1[1] - 20),
                          Point(self.posPuck1[0] + 7, self.posPuck1[1] + 20))
            puck1.setFill('white')
            puck1.draw(self.win)
    ...
    def Game(self,render,action1,action2):
        ...
        if   (self.posPuck2[0]-17 <=self.posBall[0] <= self.posPuck2[0]+17) and (self.posPuck2[1]- 30 <= self.posBall[1]<= self.posPuck2[1]+ 30) :
            self.dirBall[1] += 2 / ((self.posBall[1] - self.posPuck2[1])+0.01)
        ...
        return np.array([self.posPuck1[1],self.posPuck2[1],self.posBall[0],self.posBall[1],self.dirBall[0],self.dirBall[1]]),np.array( reward),self.done
